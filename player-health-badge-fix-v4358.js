...
function fix(){css();const root=document.querySelector('#main .view.active')||document.getElementById('main')||document.body;const badges=[...root.querySelectorAll('.swe4338-health[data-health-player]')];if(!badges.length)return;const seen=new Set();for(const b of badges){if(b.closest('.tournament-cooler-assignments')){b.remove();continue}const id=String(b.dataset.healthPlayer||'');if(!id)continue;if(seen.has(id)){b.remove();continue}seen.add(id);const name=playerName(id)||'Joueur';const unavailable=b.classList.contains('unavailable');b.classList.add('swe4358-health-fixed');b.textContent='❌ '+name+' — '+(unavailable?'Blessé indisponible':'Blessé');b.setAttribute('aria-label',name+' : '+(unavailable?'blessé indisponible':'blessé'));const row=targetFor(root,name);const host=nameHost(row,name);if(host&&b.parentElement!==host)host.appendChild(b)}}
