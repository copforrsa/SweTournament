...
function localGuard(ms=1800){window.__SWE_LOCAL_MATCH_MUTATION_UNTIL=Date.now()+ms}
...
A.addGoal=async function({match,teamId,scorerId,assisterId=null}){
 localGuard();const field=fieldForTeam(match,teamId);if(!field)throw new Error('Équipe invalide');
...
A.setAssist=async function(goal,assisterId){localGuard();const r=await sb.from('goals').update({assister_player_id:assisterId||null}).eq('id',goal.id);if(r.error)throw r.error;goal.assister_player_id=assisterId||null;emit(matchById(goal.match_id),'assist-updated',{goal});return goal};
A.deleteGoal=async function(goal){localGuard();const m=matchById(goal.match_id);if(!m)throw new Error('Match introuvable');const field=fieldForTeam(m,goal.team_id);const next=field?Math.max(0,Number(m[field]||0)-1):null;
...
A.refreshMatch=async function(matchId){localGuard(800);const [mr,gr]=await Promise.all([sb.from('matches').select('*').eq('id',matchId).single(),sb.from('goals').select('*').eq('match_id',matchId)]);if(mr.error)throw mr.error;const i=(S.matches||[]).findIndex(m=>String(m.id)===String(matchId));if(i>=0)S.matches[i]=mr.data;S.goals=[...(S.goals||[]).filter(g=>String(g.match_id)!==String(matchId)),...(gr.data||[])];emit(mr.data,'refreshed');return mr.data};
