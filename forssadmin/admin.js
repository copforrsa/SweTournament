...
async function refreshAll(){E('content').innerHTML='<div class="loading">Chargement des données Super Admin…</div>';try{const [workspaces,players,organizers,payments,complexes,catalog,actions,settings]=await Promise.all([rpc('super_admin_get_workspaces_v5'),rpc('super_admin_get_all_group_players_v2'),rpc('super_admin_get_organizer_accounts_v1'),rpc('super_admin_get_payment_overview_v1'),rpc('super_admin_get_complexes_v1'),rpc('super_admin_get_sales_catalog_v1'),rpc('super_admin_get_action_center_v1'),rpc('super_admin_get_platform_settings')]);cache={workspaces:Array.isArray(workspaces)?workspaces:[],players:Array.isArray(players)?players:[],organizers:Array.isArray(organizers)?organizers:[],payments:Array.isArray(payments)?payments:[],complexes:Array.isArray(complexes)?complexes:[],catalog:catalog||{},actions:actions||{},settings:settings||{}};render(active)}catch(e){E('content').innerHTML='<div class="card"><b>Impossible de charger certaines données.</b><div class="muted">'+esc(e.message||e)+'</div></div>'}}
function head(title,desc,action=''){return `<div class="view-title"><div><h3>${title}</h3><p class="muted">${desc}</p></div>${action}</div>`}
function render(view){if(!ready)return;({overview:renderOverview,spaces:renderSpaces,organizers:renderOrganizers,players:renderPlayers,subscriptions:renderSubscriptions,venues:renderVenues,visuals:renderVisuals,payments:renderPayments,settings:renderSettings,security:renderSecurity,support:renderSupport}[view]||renderOverview)()}
function renderOverview(){const paid=cache.workspaces.filter(w=>String(w.subscription_plan||w.plan||'free').toLowerCase()!=='free').length,acts=Number(cache.actions?.total_actions||0);E('content').innerHTML=head('Pilotage plateforme','Vue synthétique, sans rechargement automatique.','<button class="btn ghost" id="refresh">↻ Actualiser</button>')+`<div class="grid stats"><div class="card stat"><span>Espaces</span><b>${cache.workspaces.length}</b></div><div class="card stat"><span>Joueurs</span><b>${cache.players.length}</b></div><div class="card stat"><span>Organisateurs</span><b>${cache.organizers.length}</b></div><div class="card stat"><span>Offres payantes</span><b>${paid}</b></div></div><div class="card" style="margin-top:12px"><b>🔔 Centre d’actions</b><div style="font-size:28px;font-weight:900;margin-top:8px">${acts}</div><div class="muted">élément(s) nécessitant une attention</div></div>`;E('refresh').onclick=refreshAll}
function renderSpaces(){const rows=cache.workspaces.map(w=>`<tr><td><b>${esc(w.workspace_name||w.name||'Espace')}</b></td><td>${esc(w.owner_email||w.admin_email||'')}</td><td><span class="pill">${esc(String(w.subscription_plan||w.plan||'free').toUpperCase())}</span></td><td>${w.active===false||w.suspended?'Suspendu':'Actif'}</td><td>${esc(w.max_players_cap??w.max_players??'—')}</td><td>${esc(w.max_coorganizers??'—')}</td></tr>`).join('');E('content').innerHTML=head('Espaces clients','Tous les espaces et leurs limites.')+`<div class="card table-wrap"><table><thead><tr><th>Espace</th><th>Admin</th><th>Offre</th><th>Statut</th><th>Joueurs max</th><th>Co-gestionnaires</th></tr></thead><tbody>${rows||'<tr><td colspan="6">Aucun espace.</td></tr>'}</tbody></table></div>`}
function renderOrganizers(){const rows=cache.organizers.map(o=>`<tr><td><b>${esc(o.full_name||o.name||o.owner_name||'Organisateur')}</b></td><td>${esc(o.email||o.owner_email||'')}</td><td>${esc(o.workspace_count??o.workspaces_count??'—')}</td><td>${esc(o.plan_code||o.subscription_plan||'—')}</td><td>${o.launch_trial_active?'🎁 60 jours':'—'}</td></tr>`).join('');E('content').innerHTML=head('Organisateurs','Comptes organisateurs créés sur SWÉ.')+`<div class="card table-wrap"><table><thead><tr><th>Organisateur</th><th>Email</th><th>Espaces</th><th>Offre</th><th>Lancement</th></tr></thead><tbody>${rows||'<tr><td colspan="5">Aucun organisateur.</td></tr>'}</tbody></table></div>`}
