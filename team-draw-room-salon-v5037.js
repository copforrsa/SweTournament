...
(()=>{
  ...
  function renderRoom(data,message='',isError=false){
    ...
    const cards=proposals.map(proposal=>{
      const selected=isFinal?String(proposal.id)===String(selectedProposalId):String(proposal.id)===String(data.current_proposal_id);
      ...
      return '<article class="swe-v2-proposal '+(selected?'selected':'')+'" '+(isFinal?'data-swe-v2-proposal="'+esc(proposal.id)+'"':'')+'><div class="swe-v2-proposal-head"><b>Proposition '+esc(proposal.sequence)+(current?' · en cours':'')+'</b><span class="muted">'+feedback+'</span></div><div class="swe-v2-teams">'+teamHtml(proposal.snapshot)+'</div>'+voteButton+'</article>';
    }).join('')||'<div class="swe-v2-note">Le salon est ouvert. L’administrateur peut lancer la première proposition.</div>';
    const feedbackActions=data.can_vote&&!data.expired&&!isFinal&&data.current_proposal_id?'<div class="swe-v2-note"><b>Étape en cours : proposition '+esc(currentNumber)+'.</b> Avant la 5e proposition, ton avis porte uniquement sur cette composition : elle te convient ou tu demandes un nouveau tirage. Le choix de ta proposition préférée parmi toutes les compositions aura lieu à la 5e.</div><div class="swe-v2-actions"><button type="button" class="'+(data.my_feedback==='keep'?'primary':'')+'" data-swe-v2-feedback="keep">👍 Cette proposition me convient</button><button type="button" class="'+(data.my_feedback==='redraw'?'swe-v2-redraw':'')+'" data-swe-v2-feedback="redraw">🔄 Demander la proposition '+esc(Number(data.proposal_count)+1)+'</button></div>':'';
    let adminActions='';
    if(data.can_admin){
      ...
      else if(!data.expired&&Number(data.proposal_count)<5&&!isSolo)adminActions+='<span class="muted">En attente de l’avis des co-gestionnaires sur la proposition '+esc(currentNumber)+'. Une demande de nouveau tirage débloquera la proposition '+esc(Number(data.proposal_count)+1)+'.</span>';
