...
(()=>{
  ...
  function renderRoom(data,message='',isError=false){
    ...
    let adminActions='';
    if(data.can_admin){
      if(Number(data.proposal_count)<5)adminActions+='<button type="button" class="swe-v2-redraw" data-swe-v2-generate>🎲 '+(data.first_draw_pending?'Lancer la proposition 1':'Générer la proposition '+esc(Number(data.proposal_count)+1))+'</button>';
      if(selectedProposalId)adminActions+='<button type="button" class="primary" data-swe-v2-publish="'+esc(selectedProposalId)+'">✓ Publier la proposition '+esc(selectedNumber)+'</button>';
    }
