...
function paint(){try{if(typeof renderMatches==='function')renderMatches()}catch(e){console.warn('SWÉ 42.86 render',e)}}
async function refresh(force=false){if(!isMobile()||busy||typeof S==='undefined'||!S.session)return;const view=E('view-matches');if(!view?.classList.contains('active'))return;const now=Date.now();if(!force&&now-lastRefresh<1200)return;busy=true;lastRefresh=now;try{if(typeof loadTournament==='function'&&(force||dataIncomplete()||cardsMissing()))await loadTournament();paint()}catch(e){console.warn('SWÉ 42.86 sync mobile',e)}finally{busy=false}}
function schedule(force=false,delay=80){clearTimeout(timer);timer=setTimeout(()=>refresh(force),delay)}
