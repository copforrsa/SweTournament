...
(()=>{
  ...
  function apply(){
    ...
    cards.forEach((card,index)=>{
      ...
      top.classList.add('swe4477-player-top');
      ...
      let actions=top.querySelector(':scope > .swe4477-player-actions');
      if(!actions){
        const actionButtons=[...top.children].filter(child=>child.matches?.('button'));
        if(actionButtons.length){
          actions=document.createElement('div');actions.className='swe4477-player-actions';
          actionButtons.forEach(button=>actions.append(button));top.append(actions);
        }
      }
    });
  ...
  const style=document.createElement('style');style.textContent=`
    ...
    #playersList .swe4477-player-card>.swe4477-player-top{align-items:center;gap:10px}.swe4477-avatar{width:48px;height:48px;min-width:48px;border-radius:50%;object-fit:cover;border:2px solid #fff;box-shadow:0 3px 9px rgba(15,44,76,.18);background:#dbeafe}.swe4477-avatar.fallback{display:grid;place-items:center;color:#0f477c;font-size:22px}.swe4477-player-name{font-size:20px;line-height:1.15;color:#102f50}.swe4477-player-info{min-width:0}.swe4477-player-actions{display:flex;align-items:center;gap:8px;flex:0 0 auto}.swe4477-player-actions button{margin:0!important;white-space:nowrap}#playersList .swe-player-editor{align-items:stretch;gap:8px}#playersList .swe-player-team-code{align-items:center;gap:8px}.swe4477-flash{animation:swe4477Flash 1.7s ease-out}@keyframes swe4477Flash{0%,45%{box-shadow:0 0 0 4px #1d8ff2;background:#e9f6ff}100%{}}
    @media(max-width:620px){.swe4477-player-tools{top:max(6px,env(safe-area-inset-top));margin-left:-5px;margin-right:-5px;padding:9px;border-radius:14px}.swe4477-player-search{grid-template-columns:minmax(0,1fr) 126px}.swe4477-player-search label span{font-size:11px}.swe4477-player-alphabet{gap:5px;flex-wrap:nowrap;overflow-x:auto;padding-bottom:2px;scrollbar-width:none}.swe4477-player-alphabet::-webkit-scrollbar{display:none}.swe4477-player-alphabet b{position:sticky;left:0;flex:0 0 auto;background:#f8fbff;padding-right:4px}.swe4477-player-alphabet button{flex:0 0 32px;width:32px;min-height:32px}.swe4477-player-tools .swe-player-relations{flex-wrap:nowrap;overflow-x:auto;padding:8px;scrollbar-width:none}.swe4477-player-tools .swe-player-relations button{flex:0 0 auto;min-height:40px;white-space:nowrap}.swe4477-player-tools .swe-player-relations p{display:none}.swe4477-avatar{width:42px;height:42px;min-width:42px}.swe4477-player-name{font-size:18px}#playersList .swe4477-player-card>.swe4477-player-top{align-items:flex-start!important}#playersList .swe4477-player-card>.swe4477-player-top>.swe4477-player-info{flex:1 1 calc(100% - 52px)!important}#playersList .swe4477-player-actions{flex:1 0 100%;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}#playersList .swe4477-player-actions button{width:100%;min-width:0;min-height:44px;padding:8px 7px;white-space:normal}#playersList .swe-player-editor{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px!important}#playersList .swe-player-editor>input,#playersList .swe-player-editor>select{min-width:0!important;width:100%!important}#playersList .swe-player-editor>input:first-child{grid-column:1/-1}#playersList .swe-player-editor>button{grid-column:1/-1;width:100%;min-height:46px}#playersList .swe-player-team-code{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px!important}#playersList .swe-player-team-code>span{grid-column:1/-1;min-width:0;overflow-wrap:anywhere}#playersList .swe-player-team-code>button{width:100%;min-width:0;min-height:44px;padding:8px 7px;white-space:normal}}
