...
function environment(initialized=true){
 ...
 const m={id:'first',tournament_id:'test',pitch:'Carrefour',rotation_role:'king',rotation_generated:true,status:'scheduled',home_team_id:'a',away_team_id:'b',home_score:0,away_score:0};
...
test('closing repaired Carrefour resumes all three pitches using preserved previous results',async()=>{
 ...
 assert.equal(h.inserts.length,3);assert.ok(h.inserts.every(x=>x.rotation_generated===true));assert.deepEqual(h.inserts.map(x=>x.rotation_role),['king','middle','stream']);
