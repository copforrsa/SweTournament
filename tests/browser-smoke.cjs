...
(async()=>{
 ...
 try{
  ...
  await page.locator('[data-view="payments"]').click();await page.locator('#coorgPaymentRows').waitFor();assert.match(await page.locator('#coorgPaymentRows').innerText(),/coorga@example\.invalid[\s\S]*Payé par orga/);assert.match(await page.locator('#coorgPaymentRows').innerText(),/selfpay@example\.invalid[\s\S]*Payé par lui-même/);assert.match(await page.locator('#coorgPaymentRows').innerText(),/pending@example\.invalid[\s\S]*En attente de paiement/);assert.equal(await page.locator('[data-coorg-payment]').count(),3);await page.locator('#coorgPaymentSearch').fill('pending@example.invalid');assert.equal(await page.locator('[data-coorg-payment]:visible').count(),1);console.log('Super Admin co-organizer emails and real payment statuses: OK');
