 ...
 window.supabase={createClient:()=>{
  ...
  return {
   ...
   rpc:async(name,args)=>{
    ...
    if(name==='super_admin_get_coorganizer_billing_v1')return {data:[
      {workspace_id:ids.current,workspace_name:'Groupe du dimanche',organizer_email:'orga@example.invalid',coorganizer_user_id:fakeId(300),coorganizer_email:'gift@example.invalid',member_status:'active',payment_status:'gifted',payment_updated_at:'2026-09-13T09:55:00Z'},
      {workspace_id:ids.current,workspace_name:'Groupe du dimanche',organizer_email:'orga@example.invalid',invite_id:fakeId(399),coorganizer_email:'invitation-offerte@example.invalid',member_status:'invited',payment_status:'gifted',payment_updated_at:'2026-09-13T09:57:00Z'},
      {workspace_id:ids.current,workspace_name:'Groupe du dimanche',organizer_email:'orga@example.invalid',coorganizer_user_id:fakeId(301),coorganizer_email:'coorga@example.invalid',member_status:'active',payment_status:'organizer_paid',linked_player_id:data.players[0].id,linked_player_name:data.players[0].name,payment_updated_at:'2026-09-13T10:00:00Z'},
      {workspace_id:ids.current,workspace_name:'Groupe du dimanche',organizer_email:'orga@example.invalid',coorganizer_user_id:fakeId(302),coorganizer_email:'selfpay@example.invalid',member_status:'active',payment_status:'self_paid',billing_period:'month',payment_updated_at:'2026-09-13T10:05:00Z'},
      {workspace_id:ids.current,workspace_name:'Groupe du dimanche',organizer_email:'orga@example.invalid',coorganizer_email:'pending@example.invalid',member_status:'invited',payment_status:'pending_payment',billing_period:'year',payment_updated_at:'2026-09-13T10:10:00Z'}
    ]};
...
(async()=>{
 ...
 try{
  for(const viewport of [{width:1280,height:900},{width:768,height:1024},{width:360,height:800}]){
   ...
   await page.locator('[data-guest-choice="yes"]').click();assert.equal(await page.locator('#publicGuestName').isVisible(),true);
  ...
  await page.locator('[data-view="payments"]').click();await page.locator('#coorgPaymentRows').waitFor();const coorgText=await page.locator('#coorgPaymentRows').innerText();assert.match(coorgText,/gift@example\.invalid[\s\S]*Offert/);assert.match(coorgText,/invitation-offerte@example\.invalid[\s\S]*Invitation envoyée — non acceptée[\s\S]*Offert/);assert.match(coorgText,/coorga@example\.invalid[\s\S]*Payé par orga/);assert.match(coorgText,/selfpay@example\.invalid[\s\S]*Payé par lui-même/);assert.match(coorgText,/pending@example\.invalid[\s\S]*En attente de paiement/);assert.equal(await page.locator('[data-coorg-payment]').count(),5);await page.locator('#coorgPaymentSearch').fill('pending@example.invalid');assert.equal(await page.locator('[data-coorg-payment]:visible').count(),1);console.log('Super Admin co-organizer access origins and invitation lifecycle: OK');
  ...
  const stale=await context.newPage();stale.on('pageerror',e=>errors.push(e.message));await stale.route('**/build-version.js?check=*',route=>route.fulfill({contentType:'text/javascript',body:"window.SWE_BUILD_VERSION='44.00';"}));await stale.goto(base+'/?s=TESTCODE');await stale.locator('#sweBuildRefresh').waitFor();await stale.locator('#publicGuestFields summary').click();await stale.locator('#publicGuestName').fill('Saisie non rechargée');await stale.waitForTimeout(1200);assert.equal(await stale.locator('#publicGuestName').inputValue(),'Saisie non rechargée');assert.notEqual(await stale.evaluate(()=>window.SWE_BUILD_VERSION),'44.00');assert.equal(new URL(stale.url()).searchParams.get('s'),'TESTCODE');await stale.close();
  ...
  assert.match(await premium.locator('#registrationMissions').innerText(),/Vérifie les présences/);assert.equal(await premium.locator('.sp-coorg-instructions').getAttribute('open'),null);
  ...
  await premium.locator('.sp-coorg-instructions summary').click();assert.equal(await premium.locator('.sp-coorg-instructions').getAttribute('open'),'');await premium.locator('.sp-coorg-instructions summary').click();
  ...
  await premium.locator('[data-guest-choice="yes"]').click();await premium.locator('#publicGuestName').fill('Saisie à préserver');
