...
(async()=>{
 ...
 try{
  ...
  await premium.evaluate(current=>{const t=window.__snapshot.tournaments.find(t=>t.id===current);t.max_players=30;t.format='king_of_pitch';window.__snapshot.teams.push({id:'current-team-a',tournament_id:current,name:'Lions du Nord',color:'#b91c1c'},{id:'current-team-b',tournament_id:current,name:'Étoiles Royales',color:'#1d4ed8'});window.__snapshot.team_players.push(...window.__snapshot.players.slice(0,10).map((p,i)=>({team_id:i<5?'current-team-a':'current-team-b',player_id:p.id})));document.querySelector('#publicView').__registrationPresentation.update();},current);
  await premium.locator('#registrationSelectionLock').filter({hasText:'Effectif complet'}).waitFor();assert.equal(await premium.locator('#publicPlayerSelect').isDisabled(),true);assert.match(await premium.locator('.sp-royal-format').innerText(),/Roi du Terrain/);assert.match(await premium.locator('.sp-royal-head h2').innerText(),/prétendants à la couronne/i);assert.equal(await premium.locator('.sp-team-jump').getAttribute('href'),'#registrationTeamGrid');assert.ok(await premium.evaluate(()=>parseFloat(getComputedStyle(document.querySelector('.sp-team h3')).fontSize)>20));assert.ok(await premium.evaluate(()=>document.querySelector('.sp-royal-countdown').compareDocumentPosition(document.querySelector('.sp-royal-format'))&Node.DOCUMENT_POSITION_FOLLOWING));
