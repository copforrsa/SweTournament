...
function installMock(data,ids,ruleRows){
 if(window.__enableTopFive){data.features.player_ratings_enabled=true;data.features.top_player_enabled=true;for(const p of data.players.slice(4,6))data.match_player_assignments.push({match_id:ids.match,team_id:data.teams[0].id,player_id:p.id});}
 ...
 window.supabase={createClient:()=>{
  ...
  return {
   ...
   rpc:async(name,args)=>{
    ...
    if(name==='public_tournament_payment_status'){if(window.__paymentDelay)await new Promise(r=>setTimeout(r,window.__paymentDelay));return {data:window.__paymentStatus||{reason:'free',entry_fee_cents:0}};}
    if(name==='public_tournament_payment_choice_status')return {data:{}};
    if(name==='get_public_registration_player_rating')return {data:{player_id:args.p_player_id,rating_group_name:'Chien Boul Academy',is_coorganizer:args.p_player_id===data.players[0].id,avg_rating:args.p_player_id===data.players[0].id?3.3:null}};
...
(async()=>{
 ...
 try{
  ...
  const topPage=await context.newPage();await topPage.addInitScript(()=>window.__enableTopFive=true);await topPage.goto(base+'/?s=TESTCODE');await topPage.locator('#publicTournamentTopFive li').first().waitFor();assert.equal(await topPage.locator('#publicTournamentTopFive li').count(),5);assert.match(await topPage.locator('#publicTournamentTopFive').innerText(),/Tournoi précédent/);assert.match(await topPage.locator('#publicTournamentTopFive li').first().innerText(),/Joueur 2/);
  ...
  const participant=await context.newPage();await participant.goto(base+'/?s=TESTCODE');await participant.locator('#publicPlayerSelect').selectOption(players[0].id);await participant.locator('#registrationMissions').filter({hasText:'dans la soirée'}).waitFor();
  await participant.locator('#registrationPlayerStats').screenshot({path:path.join(screenshotDir,'premium-player-stats.png')});
  await participant.locator('#publicPlayerSelect').selectOption(players[1].id);await participant.locator('#registrationMissions').waitFor({state:'hidden'});assert.match(await participant.locator('#registrationPlayerStats').innerText(),/Non noté/);
  await participant.evaluate(()=>{window.__paymentStatus={available:true,entry_fee_cents:1000,total_cents:1000};window.__paymentDelay=300;});
  await participant.locator('#publicPlayerSelect').selectOption(players[0].id);await participant.locator('#publicPlayerSelect').selectOption('');await participant.waitForTimeout(800);assert.equal(await participant.locator('#publicPaymentBox').isVisible(),false);
  await participant.locator('#publicPlayerSelect').selectOption(players[0].id);await participant.locator('#publicPayEntry').waitFor();
  await participant.evaluate(()=>window.__paymentStatus={payment_status:'paid',entry_fee_cents:1000});await participant.locator('#publicPlayerSelect').selectOption(players[1].id);await participant.locator('#publicPaymentBox').filter({hasText:'Participation payée'}).waitFor();assert.equal(await participant.locator('#publicPayEntry').count(),0);await participant.close();
  const offers=await context.newPage();
  await offers.goto(base+'/');
  await offers.evaluate(()=>{const card=document.querySelector('#homeEcosystemCard');document.body.replaceChildren(card);card.classList.remove('hidden');});
  const commercialSource=fs.readFileSync(path.join(root,'app.js'),'utf8').split('function renderEcosystemCommercial(){')[1].split('function renderHome(){')[0];
  await offers.evaluate(source=>{window.__renderOffers=new Function('S','isAdmin','$','refreshCoorgPurchasePrice','return (function(){'+source.trim().replace(/}$/, '')+'})();');window.__offerState={commercialAccess:{},organizerAccess:{},workspaceFeatures:{}};window.__paintOffers=()=>window.__renderOffers(window.__offerState,()=>true,s=>document.querySelector(s),()=>{});window.__paintOffers();},commercialSource);
  assert.equal(await offers.locator('#homeBuyCoorg').isVisible(),false);assert.equal(await offers.locator('#ecoRatingsModule').isVisible(),false);
  await offers.locator('#homeCoorgOfferCard summary').click();assert.equal(await offers.locator('#homeBuyCoorg').isVisible(),true);await offers.locator('#homeCoorgQty').fill('3');
  await offers.locator('#homeModulesOffer summary').click();assert.match(await offers.locator('#ecoRatingsModule').innerText(),/À DÉCOUVRIR/);
  await offers.evaluate(()=>window.__paintOffers());assert.equal(await offers.locator('#homeCoorgQty').inputValue(),'3');assert.equal(await offers.locator('#ecoRatingsModule').isVisible(),true);
  for(const width of [1280,390]){await offers.setViewportSize({width,height:1100});await offers.locator('#homeEcosystemCard').screenshot({path:path.join(screenshotDir,'compact-offers-'+width+'.png')});assert.ok(await offers.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+2));}
  await offers.evaluate(()=>{window.__offerState.organizerAccess={trial_active:true,days_remaining:54};window.__offerState.workspaceFeatures={player_ratings_enabled:true,third_half_enabled:true,tournaments_enabled:true};window.__paintOffers();});assert.match(await offers.locator('#ecoRatingsModule').innerText(),/INCLUS DANS TON ESSAI/);assert.equal(await offers.locator('#homeCoorgOfferCard').isVisible(),false);
  await offers.evaluate(()=>{window.__offerState.commercialAccess={special_access_enabled:true};window.__paintOffers();});assert.match(await offers.locator('#ecoRatingsModule').innerText(),/ACCÈS OFFERT/);
  await offers.evaluate(()=>{window.__offerState.commercialAccess={subscription_plan:'pro'};window.__offerState.organizerAccess={};window.__paintOffers();});assert.match(await offers.locator('#ecoRatingsModule').innerText(),/ACTIF/);
  await offers.locator('#homeModulesOffer summary').click();await offers.locator('#homeCoorgOfferCard summary').click();assert.equal(await offers.locator('#ecoRatingsModule').isVisible(),false);assert.equal(await offers.locator('#homeBuyCoorg').isVisible(),false);await offers.locator('#homeEcosystemCard').screenshot({path:path.join(screenshotDir,'compact-offers-closed.png')});await offers.close();
