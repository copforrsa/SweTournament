...
function setLock(on){window.__SWE_QUICK_ASSIST_ACTIVE=!!on;}
function holdSettling(ms=1400){clearTimeout(unlockTimer);window.__SWE_QUICK_ASSIST_SETTLING_UNTIL=Date.now()+ms;setLock(true);unlockTimer=setTimeout(()=>{setLock(false);window.__SWE_QUICK_ASSIST_SETTLING_UNTIL=0;},ms)}
...
function closeStable(){removeSheet();current=null;busy=false;holdSettling(1400)}
...
function boot(){ensureStyle();bind();window.addEventListener('pageshow',()=>{if(!current){removeSheet();busy=false;if(Date.now()>=Number(window.__SWE_QUICK_ASSIST_SETTLING_UNTIL||0))setLock(false)}})}
