...
function render(data){
 ...
 if(titles){
  ...
  const winnerCard=conquestMode
    ?(winner?'<div class="live-title"><small>🏆 VAINQUEUR DE LA CONQUÊTE</small><div class="swe-award-name"><span class="swe-award-emblem" aria-hidden="true">🏆</span><b>'+esc(winner.name)+'</b></div><div class="king-match-count"><strong>🏆</strong><span>vainqueur après la finale</span></div><span>Finale · '+Number(finalMatch.home_score||0)+' - '+Number(finalMatch.away_score||0)+'</span></div>'
      :'<div class="live-title"><small>⚔️ FINALE DE LA CONQUÊTE</small><div class="swe-award-name"><span class="swe-award-emblem" aria-hidden="true">⌛</span><b>'+esc(finalMatch?(teamMap.get(String(finalMatch.home_team_id))?.name||'?')+' — '+(teamMap.get(String(finalMatch.away_team_id))?.name||'?'):'En attente des demi-finales')+'</b></div><div class="king-match-count"><strong>⌛</strong><span>le vainqueur sera celui qui remporte la finale</span></div><span>'+(finalMatch?matchState(finalMatch):'Tableau en préparation')+'</span></div>')
  ...
  const conquestCard=conquestMode?'<div class="live-title king" style="overflow:auto"><small>⚔️ TABLEAU DES RENCONTRES · CONQUÊTE</small><table class="live-table swe-conquest-table" style="margin-top:12px;color:#fff;background:transparent"><thead><tr><th style="background:transparent;color:#cbefff">Tour</th><th style="background:transparent;color:#cbefff">Rencontre</th><th style="background:transparent;color:#cbefff">Score</th></tr></thead><tbody>'+conquestMatches.map(m=>'<tr style="background:transparent;color:#fff"><td style="background:transparent;color:#fff">'+esc(String(m.round_label||'Conquête').replace(/^Conquête\\s*·\\s*/,''))+'</td><td style="background:transparent;color:#fff"><b>'+esc(teamMap.get(String(m.home_team_id))?.name||'?')+' — '+esc(teamMap.get(String(m.away_team_id))?.name||'?')+'</b><br><small style="color:#a9edff">'+matchState(m)+'</small></td><td style="background:transparent;color:#fff"><b>'+Number(m.home_score||0)+' - '+Number(m.away_score||0)+'</b></td></tr>').join('')+'</tbody></table></div>':'';
