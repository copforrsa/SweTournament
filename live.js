...
function render(data){
 ...
 if(titles){
  ...
  const finalMatch=conquestMatches.find(m=>/finale/i.test(String(m.round_label||'')))||null;
  ...
  const conquestCard=conquestMode?'<div class="live-title king" style="overflow:auto"><small>⚔️ TABLEAU DES RENCONTRES · CONQUÊTE</small><table class="live-table" style="margin-top:7px;color:#fff"><thead><tr><th style="color:#cbefff">Tour</th><th style="color:#cbefff">Rencontre</th><th style="color:#cbefff">Score</th></tr></thead><tbody>'+conquestMatches.map(m=>'<tr><td>'+esc(String(m.round_label||'Conquête').replace(/^Conquête\\s*·\\s*/,''))+'</td><td><b>'+esc(teamMap.get(String(m.home_team_id))?.name||'?')+' — '+esc(teamMap.get(String(m.away_team_id))?.name||'?')+'</b><br><small>'+matchState(m)+'</small></td><td><b>'+Number(m.home_score||0)+' - '+Number(m.away_score||0)+'</b></td></tr>').join('')+'</tbody></table></div>':'';
