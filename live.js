...
function render(data){
 ...
 if(titles){
  ...
  titles.innerHTML=(winner?'<div class="live-title"><small>🏆 VAINQUEUR DU TOURNOI</small><b>'+esc(winner.name)+'</b><span>'+winner.pts+' pts · diff. '+((winner.bp-winner.bc)>=0?'+':'')+(winner.bp-winner.bc)+'</span></div>':'')+(king?'<div class="live-title king"><small>👑 ROI DU TERRAIN · CARREFOUR</small><b>'+esc(king.name)+'</b><span>'+king.points+' pt'+(king.points>1?'s':'')+' · '+king.wins+' victoire'+(king.wins>1?'s':'')+(king.draws?' · '+king.draws+' nul'+(king.draws>1?'s':''):'')+'</span></div>':'');
 }
