...
(()=>{
  ...
  let latest=null, selected=null, opening=false,acting=false;
  ...
  function render(message='',error=false){
    ...
    const visibleVoters=asArray(state.voters).map((v,index)=>({v,index})).sort((a,b)=>Number(Boolean(b.v.decision))-Number(Boolean(a.v.decision))||a.index-b.index).slice(0,5).map(x=>x.v);
    const voterHtml=visibleVoters.map(v=>'<div class="swe-voter"><span>'+esc(v.name)+'</span><b>'+(!v.decision?'En attente':v.decision==='keep'?'Conserver':'Nouveau tirage')+'</b></div>').join('')||'<span class="muted">Aucun co-gestionnaire inscrit n’est éligible à cet avis.</span>';
    const voteActions=state.can_vote&&current?'<button type="button" class="primary" data-swe-vote="keep">👍 Conserver cette proposition</button><button type="button" data-swe-vote="redraw">🔄 Demander un nouveau tirage</button>':'';
    const adminActions=state.can_admin&&state.first_draw_pending?'<button type="button" class="primary" data-swe-redraw>🎲 Lancer le premier tirage</button>':state.can_admin?'<button type="button" class="swe-redraw" data-swe-redraw '+(Number(state.redraws_used||0)>=Number(state.max_redraws||0)?'disabled':'')+'>🎲 Lancer un nouveau tirage</button><button type="button" class="primary" data-swe-publish="'+esc(current||'')+'">✓ Choisir et publier cette composition</button>':'';
    root.innerHTML='<section class="swe-room"><div class="swe-room-head"><div><div style="font-size:11px;letter-spacing:.08em;font-weight:900;color:#1762bf">SALON PRIVÉ DE TIRAGE</div><h2 class="sectiontitle" style="margin:4px 0">🗳️ '+esc(state.tournament_name||'Composition des équipes')+'</h2>'+deadline+'</div><button type="button" aria-label="Fermer" data-swe-close>×</button></div>' +(message?'<div class="swe-room-note '+(error?'swe-room-error':'')+'">'+esc(message)+'</div>':'')+'<div class="swe-room-grid"><div><p class="muted">Les propositions ne sont pas visibles par les joueurs tant que l’administrateur ne les publie pas.</p>'+proposalHtml+'<div class="swe-room-actions">'+voteActions+adminActions+'<button type="button" data-swe-refresh-teams>↻ Actualiser les équipes</button><button type="button" data-swe-refresh>Actualiser les avis</button></div>'+ (state.can_admin?'<p class="muted" style="margin:10px 0 0">Nouveaux tirages : '+esc(state.redraws_used||0)+' / '+esc(state.max_redraws||0)+'. Les matchs déjà créés bloquent volontairement tout nouveau tirage.</p>':'')+'</div><aside class="swe-voters"><b>Co-gestionnaires inscrits</b><p class="muted" style="margin:5px 0 8px">Leur avis éclaire la décision de l’administrateur.</p>'+voterHtml+'</aside></div></section>';
  }
  async function refresh(tid){try{latest=await call(isAdminUser()?'team_draw_room_open_v1':'team_draw_room_join_v1',{p_tournament_id:tid});render()}catch(err){render(err.message||'Le salon ne peut pas être ouvert.',true)}}
  ...
  async function action(button){const root=E('sweDrawRoom4453'), tid=latest?.tournament_id;if(!tid||acting)return;acting=true;root?.querySelectorAll('button').forEach(b=>b.disabled=true);try{
  ...
  document.addEventListener('click',event=>{
    ...
    if(acting){event.preventDefault();event.stopImmediatePropagation();return;}
