...
(()=>{
  ...
  function render(message='',error=false){
    ...
    const adminActions=state.can_admin?'<button type="button" class="swe-redraw" data-swe-redraw '+(Number(state.redraws_used||0)>=Number(state.max_redraws||0)?'disabled':'')+'>🎲 Lancer un nouveau tirage</button><button type="button" class="primary" data-swe-publish="'+esc(current||'')+'">✓ Choisir et publier cette composition</button>':'';
  ...
  document.addEventListener('click',event=>{
    const target=event.target.closest?.('[data-swe-open-draw-room],[data-coorg-action="team"],[data-swe-close],[data-swe-proposal],[data-swe-vote],[data-swe-redraw],[data-swe-publish],[data-swe-refresh],[data-swe-refresh-teams]');if(!target)return;
