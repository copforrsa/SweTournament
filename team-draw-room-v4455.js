...
(()=>{
  ...
  function render(message='',error=false){
    ...
    const visibleVoters=asArray(state.can_admin?state.voters:[]).map((v,index)=>({v,index})).sort((a,b)=>Number(Boolean(b.v.decision))-Number(Boolean(a.v.decision))||a.index-b.index).slice(0,5).map(x=>x.v);
