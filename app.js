    ...
    d.innerHTML=
      ...
      '<div class="sa-settings-block">'+
        ...
        '<div class="sa-linked-options">'+
          ...
          '<div class="sa-linked-option review"><span>🗳️</span><div><b>Validation collaborative des équipes</b><small>Incluse avec STANDARD/PRO. Les co-gestionnaires présents peuvent valider ou demander un nouveau tirage pendant 1 h.</small></div><strong>'+(w.team_review_enabled?'ACTIF':'INACTIF')+'</strong></div>'+
        '</div>'+
      '</div>'+
...
function renderTeamReviewPanel(t){
  ...
  const r=S.teamReviewState||{status:t.team_review_status||'not_started',max_redraws:Number(t.max_team_redraws??4),redraws_used:Number(t.team_redraws_used||0),eligible_count:0,validate_votes:0,redraw_votes:0};
  ...
  if(isAdmin()){
    html+='<div class="team-review-admin"><label><span>Nombre maximum de nouveaux tirages</span><input id="teamReviewMaxRedraws" type="number" min="0" max="4" value="'+Math.min(4,Number(r.max_redraws ?? t.max_team_redraws ?? 4))+'"></label><button id="saveTeamReviewLimit">💾 Enregistrer la limite</button>'+
  ...
  const save=$('#saveTeamReviewLimit');if(save)save.onclick=async()=>{const max=Math.max(0,Math.min(4,Number($('#teamReviewMaxRedraws').value)||0));save.disabled=true;const {error}=await sb.rpc('admin_configure_tournament_team_review',{p_tournament_id:t.id,p_max_redraws:max,p_requested:true,p_duration_minutes:60});save.disabled=false;if(error)return toast(error.message);await loadTournament();renderTeams();toast('Limite de nouveaux tirages enregistrée ✅');};
...
async function runSmartTeamGeneration(isRedraw=false){
  ...
  try{
    ...
    if(t.format!=='league'&&t.team_review_requested&&S.workspaceFeatures.team_review_enabled&&isAdmin()){
      const configured=await sb.rpc('admin_configure_tournament_team_review',{p_tournament_id:targetId,p_max_redraws:Math.min(4,Number(t.max_team_redraws??4)),p_requested:true,p_duration_minutes:60});
...
async function loadPublicPage(token,bootState){
  ...
  function publishPublicCompositionContext(){
    window.__swePublicCompositionContext={tournament:regTour,tournaments,teams,teamPlayers,players};
    document.dispatchEvent(new Event('swe:public-composition-ready'));
  }
  publishPublicCompositionContext();
  const regBox=$('#publicRegistration');
  let registrationPresentation=null;
  let registrationCoorganizers=[],registrationRatingWindows=[],registrationIsCoorganizer=false,registrationIsAdmin=false,registrationPersonalInstruction='';
  ...
  async function loadPresentationMetadata(){
    ...
    registrationIsCoorganizer=false;registrationIsAdmin=false;registrationCoorganizers=[];registrationRatingWindows=[];registrationPersonalInstruction='';
    if(!session.data?.session){registrationPresentation?.update();return;}
    const {data,error}=await sb.rpc('get_my_tournament_presentation_v1',{p_tournament_id:regTour.id});
    if(error||!data){registrationPresentation?.update();return;}
    registrationIsCoorganizer=data.is_coorganizer===true;registrationIsAdmin=data.is_admin===true;
