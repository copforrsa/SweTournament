...
function renderTournaments(){
  ...
  current.forEach(t=>{
    ...
    if(hasAdminOps()){
      ...
      coolerAssignments.innerHTML='<div><b>🧊 Répartir les missions Glacière</b><div class="muted" style="margin-top:4px">Tu peux choisir un membre du groupe ou un non-membre. L’inscription est obligatoire uniquement pour ajouter un lien de paiement SWÉ.</div></div><div class="grid g2" style="margin-top:10px"><label><span class="muted">Responsable de la glacière</span><select data-admin-cooler-player>'+assignmentOptions(assignment.cooler_player_id,'Choisir un responsable')+'</select></label><label><span class="muted">Responsable des glaçons</span><select data-admin-ice-player>'+assignmentOptions(assignment.ice_player_id,'Choisir une personne')+'</select></label></div><div data-admin-cooler-payment-note class="readonly-note" style="margin-top:9px"></div><button type="button" class="primary" data-save-third-half-assignments style="margin-top:9px">Enregistrer les responsables</button>';
      const coolerSelect=coolerAssignments.querySelector('[data-admin-cooler-player]'),iceSelect=coolerAssignments.querySelector('[data-admin-ice-player]'),paymentNote=coolerAssignments.querySelector('[data-admin-cooler-payment-note]');
      const updatePaymentNote=()=>{const id=coolerSelect.value;if(!id){paymentNote.textContent='Choisis le responsable de la glacière.';return;}const name=candidates.find(p=>String(p.id)===String(id))?.name||'Cette personne';paymentNote.innerHTML=registeredIds.has(String(id))?'✅ <b>'+esc(name)+'</b> est inscrit : il pourra ajouter son lien personnel depuis la page d’inscription.':'ℹ️ <b>'+esc(name)+'</b> n’est pas inscrit : il gérera la glacière hors plateforme et aucun lien de paiement ne sera proposé.';};
      coolerSelect.onchange=updatePaymentNote;updatePaymentNote();
      coolerAssignments.querySelector('[data-save-third-half-assignments]').onclick=async e=>{const b=e.currentTarget;b.disabled=true;const {data,error}=await sb.rpc('admin_save_third_half_assignments_v1',{p_tournament_id:t.id,p_cooler_player_id:coolerSelect.value||null,p_ice_player_id:iceSelect.value||null});b.disabled=false;if(error)return toast(error.message);await loadAll();toast(data?.responsible_registered?'Responsables enregistrés • lien SWÉ autorisé ✅':'Responsables enregistrés • gestion hors plateforme ✅');};
    ...
    function renderPublicThirdHalfRegistration(){
      ...
      if(!state.responsible_player_id){
        ...
        if(state.responsible_registered===false){
          action='<div class="third-half-owner-locked"><b>'+esc(ownerName)+', tu es responsable de la glacière.</b><p>Tu n’es pas inscrit à ce Swé : la mission est conservée, mais elle sera gérée hors plateforme. Aucun compte ni lien de paiement SWÉ n’est demandé.</p><small>Pour intégrer ton lien personnel, inscris-toi d’abord au tournoi puis reviens sur cette page.</small></div>'+logistics;
        }else{
