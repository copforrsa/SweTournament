...
const unitCents=(period:string)=>period==='year'?1990:199;
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const billingPeriod=body.billing_period==='year'?'year':'month';
    ...
    const unit=unitCents(billingPeriod);
    const session=await stripe.checkout.sessions.create({
      mode:'subscription',customer_email:user.email||undefined,client_reference_id:workspaceId,
      line_items:[{quantity,price_data:{currency:'eur',unit_amount:unit,recurring:{interval:billingPeriod==='year'?'year':'month'},product_data:{name:'SWÉ — Co-gestionnaire supplémentaire',description:`${quantity} accès co-gestionnaire(s) supplémentaire(s)`}}}],
      success_url:successUrl,cancel_url:cancelUrl,allow_promotion_codes:false,
      metadata:{type:'swe_coorganizers',workspace_id:workspaceId,quantity:String(quantity),unit_amount_cents:String(unit),billing_period:billingPeriod,request_id:clientRequestId,actor_user_id:user.id},
      subscription_data:{metadata:{type:'swe_coorganizers',workspace_id:workspaceId,quantity:String(quantity),unit_amount_cents:String(unit),billing_period:billingPeriod,request_id:clientRequestId,actor_user_id:user.id}},
