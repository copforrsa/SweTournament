create or replace function private.is_own_rating_player_v4463(p_player_id uuid)
returns boolean language sql stable security invoker set search_path='' as $$
 select auth.uid() is not null and exists(
 select 1 from public.players p where p.id=p_player_id and (
 exists(select 1 from public.workspace_members wm where wm.workspace_id=p.workspace_id and wm.user_id=auth.uid() and wm.linked_player_id=p.id)
 or exists(select 1 from public.global_player_profiles gp where gp.id=p.global_player_id and gp.user_id=auth.uid())
 ));
$$;
revoke all on function private.is_own_rating_player_v4463(uuid) from public;
