-- Assigned test matches reuse the existing isolated matches and the workspace score permissions.
alter table private.match_test_runs
 add column target_workspace_id uuid references public.workspaces(id) on delete set null,
 add column manager_user_id uuid references auth.users(id) on delete set null;
create index match_test_runs_manager_workspace_idx on private.match_test_runs(manager_user_id,target_workspace_id,created_at desc);

-- Recipient lookup mirrors coorganizer_has_permission('scores') and has_temporary_admin.
create function private.test_match_recipient_allowed(p_workspace_id uuid,p_user_id uuid)
...
create function private.can_manage_test_match(p_match_id uuid)
returns boolean language sql stable security definer set search_path='' as $$
...
$$;
revoke all on function private.can_manage_test_match(uuid) from public,anon,authenticated;

create function public.super_admin_get_test_match_recipients()
returns jsonb language plpgsql security definer set search_path='' as $$
begin
 if not private.is_platform_super_admin() then raise exception 'Accès Super Admin requis'; end if;
 return coalesce((select jsonb_agg(to_jsonb(x) order by x.workspace_name,x.name) from (
  select wm.workspace_id,w.name as workspace_name,wm.user_id,wm.role,
   coalesce(nullif(g.display_name,''),g.public_player_id,'Gestionnaire') as name,g.public_player_id as swe_id
  from public.workspace_members wm join public.workspaces w on w.id=wm.workspace_id
  left join public.global_player_profiles g on g.user_id=wm.user_id
  where private.test_match_recipient_allowed(wm.workspace_id,wm.user_id)
 ) x),'[]'::jsonb);
end $$;

drop function public.super_admin_create_test_match(text[],uuid);
CREATE OR REPLACE FUNCTION public.super_admin_create_test_match(p_swe_ids text[], p_request_id uuid, p_target_workspace_id uuid DEFAULT NULL, p_manager_user_id uuid DEFAULT NULL)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
create function public.get_my_workspace_test_matches(p_workspace_id uuid)
returns jsonb language plpgsql security definer set search_path='' as $$
begin
...
revoke all on function public.super_admin_get_test_match_recipients() from public,anon;
revoke all on function public.super_admin_create_test_match(text[],uuid,uuid,uuid) from public,anon;
revoke all on function public.get_my_workspace_test_matches(uuid) from public,anon;
grant execute on function public.super_admin_get_test_match_recipients() to authenticated;
grant execute on function public.super_admin_create_test_match(text[],uuid,uuid,uuid) to authenticated;
grant execute on function public.get_my_workspace_test_matches(uuid) to authenticated;
