-- Accepte les nouveaux apports matériels proposés par le formulaire public.
...
begin
  ...
  if not exists (
    ...
    where t.id = p_tournament_id and w.public_token = p_token and w.public_enabled = true
      and t.third_half_active = true and tp.present = true and tp.registration_status <> 'cancelled'
  ...
  if p_participating then
    ...
    if v_mode = 'supplies' and v_item not in ('cooler', 'ice', 'beers_3', 'beers_6', 'beers_12', 'ti_punch', 'fruits', 'cups', 'soft_drinks', 'snacks', 'other') then raise exception 'Choisis ce que tu souhaites apporter'; end if;
  end if;
  ...
  where tournament_id = p_tournament_id and player_id = p_player_id and present = true and registration_status <> 'cancelled';
