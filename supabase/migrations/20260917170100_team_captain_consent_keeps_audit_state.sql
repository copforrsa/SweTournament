-- Preserve the accepted invitation row so the public roster can explain why a
-- member is already confirmed (captain agreement or member acceptance).
...
begin
  select array_agg(distinct x) into v_ids from unnest(coalesce(p_player_ids,'{}'::uuid[])) x where x<>p_player_id;
  select array_agg(distinct x) into v_confirmed_ids from unnest(coalesce(p_confirmed_player_ids,'{}'::uuid[])) x where x<>p_player_id;
  if exists(select 1 from unnest(coalesce(v_confirmed_ids,'{}'::uuid[])) x where not (x=any(coalesce(v_ids,'{}'::uuid[])))) then raise exception 'Les membres confirmés doivent faire partie de la composition'; end if;
  v_team:=public.public_create_team_with_member_code_v2(p_token,p_tournament_id,p_player_id,p_code,p_team_name,p_team_color,v_ids,p_guest_names);
  foreach v_pid in array coalesce(v_confirmed_ids,'{}'::uuid[]) loop
    update public.team_player_invitations set status='accepted',responded_at=now()
