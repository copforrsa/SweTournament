(()=>{
'use strict';
if(window.__SWE_COORG_TAB_RIGHTS_4320)return;window.__SWE_COORG_TAB_RIGHTS_4320=true;
const norm=s=>String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/\s+/g,' ').trim();
function css(){if(document.getElementById('sweCoorgRights4320Style'))return;const s=document.createElement('style');s.id='sweCoorgRights4320Style';s.textContent=`
...
function quickView(btn){
 ...
 if(t.includes('profil'))return'myplayer';
 return'';
}
function toast(){let t=document.getElementById('sweRightsToast4320');if(!t){t=document.createElement('div');t.id='sweRightsToast4320';t.style.cssText='position:fixed;z-index:9999;left:50%;top:18px;transform:translateX(-50%);padding:10px 14px;border-radius:12px;background:#071a35;color:white;font-weight:850;box-shadow:0 10px 28px rgba(0,0,0,.22);display:none';t.textContent="Cet accès n’est pas autorisé pour ton profil co-gestionnaire.";document.body.appendChild(t)}t.style.display='block';clearTimeout(t._timer);t._timer=setTimeout(()=>t.style.display='none',2200)}
...
function forceProfileEnabled(){document.querySelectorAll('.tabs button[data-view]').forEach(btn=>{if(isProfileView(btn.dataset.view))mark(btn,true)})}
function apply(){
...
function boot(){apply();let n=0;const t=setInterval(()=>{n++;apply();if(n>20)clearInterval(t)},250)}
