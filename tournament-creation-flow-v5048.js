...
function insertStyle(){
  ...
  #sweTournamentHub5050{margin:0 0 16px;padding:22px;border:1px solid #d7e7f5;border-radius:20px;background:linear-gradient(118deg,#08234a 0%,#154a91 58%,#11b9cf 100%);box-shadow:0 16px 35px rgba(14,61,109,.15);color:#fff}.swetour-hub-top{display:flex;align-items:flex-start;justify-content:space-between;gap:18px}.swetour-hub-kicker{color:#f9d56b;font-size:11px;font-weight:900;letter-spacing:1.2px;text-transform:uppercase}.swetour-hub-title{margin:5px 0 4px;font-size:26px;line-height:1.1}.swetour-hub-copy{margin:0;color:#deefff;max-width:620px;font-size:13px;line-height:1.5}.swetour-hub-create{border:0!important;background:linear-gradient(135deg,#f5bd20,#ffdf67)!important;color:#102d55!important;font-weight:900;white-space:nowrap;box-shadow:0 8px 18px rgba(0,0,0,.18)!important}.swetour-hub-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:18px}.swetour-hub-stat{padding:12px 13px;border:1px solid rgba(255,255,255,.2);border-radius:13px;background:rgba(7,24,54,.22)}.swetour-hub-stat b{display:block;font-size:21px;color:#fff}.swetour-hub-stat span{display:block;color:#dbeeff;font-size:11px;font-weight:700;margin-top:3px}.swetour-hub-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:14px}.swetour-hub-tour{appearance:none;text-align:left;border:1px solid rgba(255,255,255,.28);border-radius:13px;background:rgba(255,255,255,.11);color:#fff;padding:12px;cursor:pointer;min-width:0}.swetour-hub-tour:hover,.swetour-hub-tour.active{background:rgba(255,255,255,.22);border-color:#ffe17a}.swetour-hub-tour b,.swetour-hub-tour small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.swetour-hub-tour small{margin-top:5px;color:#dceeff;font-size:11px}.swetour-hub-empty{margin-top:15px;padding:12px;border-radius:12px;background:rgba(255,255,255,.12);color:#e3f4ff;font-size:12px}@media(max-width:700px){#sweTournamentHub5050{padding:17px}.swetour-hub-top{flex-direction:column}.swetour-hub-create{width:100%}.swetour-hub-stats,.swetour-hub-list{grid-template-columns:1fr}.swetour-hub-title{font-size:23px}}
  #tournamentList{display:grid;gap:16px}.tournament-current-card{border:1px solid #d9e7f3!important;border-radius:18px!important;padding:18px!important;background:linear-gradient(135deg,#fff,#f4faff)!important;box-shadow:0 10px 26px rgba(16,62,107,.08)}.tournament-current-head{gap:14px}.tournament-current-name{font-size:22px!important;color:#092c58}.tournament-info-grid{gap:10px!important}.tournament-info-item{border-radius:12px!important;background:#f8fbff!important;border:1px solid #e0ecf5!important;padding:11px!important}.tournament-post-actions{border:1px solid #dce8f2;border-radius:12px;padding:0 12px;background:#fff;margin-top:12px}.tournament-post-actions summary{padding:12px 0;font-weight:800;color:#16446f}.tournament-post-buttons{padding-bottom:12px}
...
function creationIssue(){
  const date=E('tourDate')?.value||'';
  const deadline=E('tourRegistrationDeadline')?.value||'';
  const complex=E('tourComplex')?.value||'';
  const pitches=[...document.querySelectorAll('.tourPitchCheck:checked')];
  const time=E('tourStartTime')?.value||'';
  const fee=E('tourEntryFee')?.value;
  if(!date)return {step:0,message:'Choisis la date du tournoi.'};
  if(!deadline)return {step:0,message:'Indique la date et l’heure de fin des inscriptions.'};
  if(!complex)return {step:1,message:'Choisis le complexe sportif avant de créer le tournoi.'};
  if(!pitches.length)return {step:1,message:'Choisis au moins un terrain réservé.'};
  if(!time)return {step:1,message:'Indique l’heure de réservation.'};
  if(fee===''||Number(fee)<0)return {step:2,message:'Indique le prix par participant.'};
  const discovery=E('tourDiscoveryMode')?.value||'unlisted';
  if(discovery!=='unlisted'&&!E('tourAllowExternal')?.checked)return {step:2,message:'Active les joueurs externes pour rendre ce SWÉ visible.'};
  if(E('tourAdminPaymentLinkEnabled')?.checked&&(!E('tourAdminPaymentProvider')?.value||!E('tourAdminPaymentLink')?.value?.trim()))return {step:2,message:'Ajoute la solution et le lien de paiement avant de l’afficher.'};
  return null;
}
function focusStepError(issue){
  if(!issue)return;
  const target=[E('tourDate'),E('tourComplex'),E('tourMaxPlayers')][issue.step];
...
function renderHub(){
  ...
  let hub=E('sweTournamentHub5050');
  if(!hub){hub=document.createElement('section');hub.id='sweTournamentHub5050';list.parentElement?.insertBefore(hub,list);}
  const tournaments=(S.tournaments||[]).filter(t=>t.format!=='league');
  const active=tournaments.filter(t=>t.status!=='finished');
  const drafts=active.filter(t=>t.status==='draft');
  const finished=tournaments.filter(t=>t.status==='finished');
  const visible=active.slice().sort((a,b)=>String(a.tournament_date||'').localeCompare(String(b.tournament_date||''))).slice(0,3);
  hub.innerHTML='<div class="swetour-hub-top"><div><div class="swetour-hub-kicker">Organisation SWÉ</div><h2 class="swetour-hub-title">Mes tournois, au même endroit</h2><p class="swetour-hub-copy">Crée un SWÉ pas à pas, puis retrouve ses inscriptions, équipes, Glacière, liens, matchs, rapports et archives dans son espace dédié.</p></div>'+(canManage()?'<button type="button" class="swetour-hub-create" data-swe-open-create>＋ Créer un tournoi</button>':'')+'</div><div class="swetour-hub-stats"><div class="swetour-hub-stat"><b>'+active.length+'</b><span>tournoi'+(active.length>1?'s':'')+' à gérer</span></div><div class="swetour-hub-stat"><b>'+drafts.length+'</b><span>brouillon'+(drafts.length>1?'s':'')+' à préparer</span></div><div class="swetour-hub-stat"><b>'+finished.length+'</b><span>archive'+(finished.length>1?'s':'')+'</span></div></div>'+(visible.length?'<div class="swetour-hub-list">'+visible.map(t=>'<button type="button" class="swetour-hub-tour '+(String(t.id)===String(S.activeTour)?'active':'')+'" data-swe-hub-tour="'+esc(t.id)+'"><b>'+esc(t.name||'SWÉ du '+t.tournament_date)+'</b><small>'+esc(t.tournament_date||'')+' · '+(t.status==='draft'?'Brouillon':'En préparation')+'</small></button>').join('')+'</div>':'<div class="swetour-hub-empty">Ton prochain SWÉ commencera ici. Crée-le en quelques étapes, puis ouvre les inscriptions lorsque tu es prêt.</div>');
  hub.querySelector('[data-swe-open-create]')?.addEventListener('click',()=>E('newTournamentToggle')?.click());
  hub.querySelectorAll('[data-swe-hub-tour]').forEach(button=>button.addEventListener('click',()=>switchTournament(button.dataset.sweHubTour)));
}
function mount(){
...
function schedule(){setTimeout(()=>{renderHub();mount();renderSwitcher();},80);}
