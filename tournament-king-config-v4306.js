...
function ensureBox(){
 ...
 box.innerHTML='<b>👑 Format Roi du terrain</b><div class="muted" style="margin:5px 0 10px">Définis le terrain du Roi et le terrain Ruisseau. Avec 3 terrains, le terrain restant devient automatiquement le DMZ / intermédiaire.</div><div class="grid g2"><label><span class="muted">👑 Terrain du Roi</span><select id="sweKingPitch"><option value="">Choisir</option></select></label><label><span class="muted">🌊 Terrain Ruisseau</span><select id="sweStreamPitch"><option value="">Choisir</option></select></label></div><div id="sweMiddlePitchHint" class="muted" style="margin-top:8px"></div>';
...
function refresh(){
 ...
 if(ids.length===2&&k&&s){if(!k.value&&ids[0])k.value=ids[0];if(!s.value){const other=ids.find(id=>id!==k.value);s.value=other||''}}
 updateHint();
}
function updateHint(){const ids=selectedPitches(),k=$('#sweKingPitch')?.value||'',low=$('#sweStreamPitch')?.value||'',h=$('#sweMiddlePitchHint');if(!h)return;if(ids.length<2){h.textContent='Sélectionne au moins 2 terrains pour le format Roi du terrain.';return}if(k&&low&&k===low){h.textContent='⚠️ Le terrain du Roi et le Ruisseau doivent être différents.';return}if(ids.length===3&&k&&low){const mid=ids.find(id=>id!==k&&id!==low);h.textContent=mid?'DMZ / terrain intermédiaire : '+pitchLabel(mid):'Choisis deux terrains différents.'}else if(ids.length===2&&k&&low){h.textContent='Deux terrains actifs : 👑 '+pitchLabel(k)+' + 🌊 '+pitchLabel(low)+' • aucun DMZ.'}else h.textContent=''}
function config(){const ids=selectedPitches(),king=$('#sweKingPitch')?.value||'',low=$('#sweStreamPitch')?.value||'';if($('#tourFormat')?.value!=='king_of_pitch')return {mode:'standard'};if(ids.length<2||ids.length>3)throw new Error('Le Roi du terrain nécessite 2 ou 3 terrains.');if(!king||!ids.includes(king))throw new Error('Choisis le terrain du Roi.');if(!low||!ids.includes(low)||low===king)throw new Error('Choisis un terrain Ruisseau différent du terrain du Roi.');if(ids.length===2)return {mode:'king_of_pitch',king,middle:null,stream:low};const remaining=ids.filter(id=>id!==king&&id!==low);return {mode:'king_of_pitch',king,middle:remaining[0]||null,stream:low}}
