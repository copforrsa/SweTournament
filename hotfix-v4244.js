...
async function purgeLegacyClient(){
  ...
  try{
    if(window.caches){
      ...
      await Promise.all(keys.filter(k=>/swe|tournoi-foot/i.test(k)).map(k=>caches.delete(k)));
    }
  }catch(_){}
}
...
function installAuthorityObserver(){
  ...
  observer=new MutationObserver(()=>{
    if(enforcing)return;
    let needs=false;
    if(/V42\.(?!50\b)\d+/.test(document.title))needs=true;
    if(!needs){
      document.querySelectorAll('h1 span,.build-badge').forEach(el=>{
        const t=(el.textContent||'').trim();
        if(/^V42\./.test(t)&&t!=='V42.50')needs=true;
        if(/^MAJ 42\./.test(t)&&t!=='MAJ 42.50')needs=true;
      });
    }
    if(needs)queueMicrotask(applyBuild);
  });
