...
function installAuthorityObserver(){if(observer||!document.documentElement)return;observer=new MutationObserver(()=>{if(enforcing)return;let needs=/V42\.(?!62\b)\d+/.test(document.title);if(!needs){document.querySelectorAll('h1 span,.build-badge').forEach(el=>{const t=(el.textContent||'').trim();if(/^V42\./.test(t)&&t!=='V42.62')needs=true;if(/^MAJ 42\./.test(t)&&t!=='MAJ 42.62')needs=true})}if(needs)queueMicrotask(applyBuild)});observer.observe(document.documentElement,{subtree:true,childList:true,characterData:true})}
...
async function boot(){
 ...
 // L'application normale ne charge plus aucun renderer Super Admin. /forssadmin est autonome.
 await load('/admin-access-gate-v4258.js?v=4262','admin-access-gate');
 await load('/hotfix-v4250-invite-auth.js?v=4262','invite-auth');
 await load('/legacy-hotfix-v4244.js?v=4262','legacy4244');
 await load('/hotfix-v4246.js?v=4262','v4246');
 await load('/hotfix-v4248.js?v=4262','v4248');
 await load('/hotfix-v4250.js?v=4262','v4250');
 await load('/hotfix-v4252-login-offers.js?v=4262','v4252-login-offers');
 await load('/hotfix-v4254-auth-stability.js?v=4262','v4254-auth-stability');
 await load('/hotfix-v4262-quickscore.js?v=4262','v4262-quickscore');
