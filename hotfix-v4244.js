...
function installAuthorityObserver(){if(observer||!document.documentElement)return;observer=new MutationObserver(()=>{if(enforcing)return;let needs=/V42\.(?!60\b)\d+/.test(document.title);if(!needs){document.querySelectorAll('h1 span,.build-badge').forEach(el=>{const t=(el.textContent||'').trim();if(/^V42\./.test(t)&&t!=='V42.60')needs=true;if(/^MAJ 42\./.test(t)&&t!=='MAJ 42.60')needs=true;});}if(needs)queueMicrotask(applyBuild);});observer.observe(document.documentElement,{subtree:true,childList:true,characterData:true});}
...
async function boot(){
 ...
 await load('/admin-access-gate-v4258.js?v=4260','admin-access-gate');
 await load('/hotfix-v4250-invite-auth.js?v=4260','invite-auth');
 await load('/legacy-hotfix-v4244.js?v=4260','legacy4244');
 await load('/hotfix-v4246.js?v=4260','v4246');
 await load('/hotfix-v4248.js?v=4260','v4248');
 await load('/hotfix-v4250.js?v=4260','v4250');
 if(window.SWE_ADMIN_ENTRY_ACTIVE===true){
   await load('/hotfix-v4250-superadmin.js?v=4260','v4250-superadmin');
   await load('/superadmin-router-v4257.js?v=4260','superadmin-router-v4257');
   await load('/hotfix-v4250-superadmin-stable.js?v=4260','v4250-superadmin-renderers');
   await load('/hotfix-v4250-superadmin-controls.js?v=4260','v4250-superadmin-controls');
   await load('/hotfix-v4250-superadmin-subscriptions.js?v=4260','v4250-superadmin-subscriptions');
   await load('/hotfix-v4250-superadmin-editors.js?v=4260','v4250-superadmin-editors');
   await load('/hotfix-v4250-superadmin-commerce.js?v=4260','v4250-superadmin-commerce');
   await load('/superadmin-entry-shell-v4260.js?v=4260','superadmin-entry-shell');
 }
 await load('/hotfix-v4252-login-offers.js?v=4260','v4252-login-offers');
 await load('/hotfix-v4254-auth-stability.js?v=4260','v4254-auth-stability');
