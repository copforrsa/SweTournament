...
function installAuthorityObserver(){if(observer||!document.documentElement)return;observer=new MutationObserver(()=>{if(enforcing)return;let needs=/V42\.(?!71\b)\d+/.test(document.title);if(!needs){document.querySelectorAll('h1 span,.build-badge').forEach(el=>{const t=(el.textContent||'').trim();if(/^V42\./.test(t)&&t!=='V42.71')needs=true;if(/^MAJ 42\./.test(t)&&t!=='MAJ 42.71')needs=true})}if(needs)queueMicrotask(applyBuild)});observer.observe(document.documentElement,{subtree:true,childList:true,characterData:true})}
...
async function boot(){
 ...
 await load('/admin-access-gate-v4258.js?v=4271','admin-access-gate');
 await load('/hotfix-v4250-invite-auth.js?v=4271','invite-auth');
 await load('/legacy-hotfix-v4244.js?v=4271','legacy4244');
 await load('/hotfix-v4246.js?v=4271','v4246');
 await load('/hotfix-v4248.js?v=4271','v4248');
 await load('/hotfix-v4250.js?v=4271','v4250');
 await load('/hotfix-v4252-login-offers.js?v=4271','v4252-login-offers');
 await load('/hotfix-v4254-auth-stability.js?v=4271','v4254-auth-stability');
 await load('/hotfix-v4262-quickscore.js?v=4271','v4262-quickscore');
 await load('/hotfix-v4263-public-registration-guard.js?v=4271','v4263-public-registration-guard');
 await load('/hotfix-v4266-public-payment-maps.js?v=4271','v4266-public-payment-maps');
 await load('/hotfix-v4268-flex-format-payment.js?v=4271','v4268-flex-format-payment');
 await load('/hotfix-v4269-rating-update.js?v=4271','v4269-rating-update');
 await load('/hotfix-v4270-coorg-fun.js?v=4271','v4270-coorg-fun');
 await load('/hotfix-v4271-whatsapp-avatar.js?v=4271','v4271-whatsapp-avatar');
