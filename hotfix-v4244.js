...
function installAuthorityObserver(){if(observer||!document.documentElement)return;observer=new MutationObserver(()=>{if(enforcing)return;let needs=/V42\.(?!50\b)\d+/.test(document.title);if(!needs){document.querySelectorAll('h1 span,.build-badge').forEach(el=>{const t=(el.textContent||'').trim();if(/^V42\./.test(t)&&t!=='V42.50')needs=true;if(/^MAJ 42\./.test(t)&&t!=='MAJ 42.50')needs=true;});}if(needs)queueMicrotask(applyBuild);});observer.observe(document.documentElement,{subtree:true,childList:true,characterData:true});}
...
async function boot(){applyBuild();installAuthorityObserver();purgeLegacyClient();await load('./legacy-hotfix-v4244.js?v=4250','legacy4244');await load('./hotfix-v4246.js?v=4250','v4246');await load('./hotfix-v4248.js?v=4250','v4248');await load('./hotfix-v4250.js?v=4250','v4250');await load('./hotfix-v4250-superadmin.js?v=4250d','v4250-superadmin');applyBuild();setTimeout(applyBuild,250);setTimeout(applyBuild,1000);}
