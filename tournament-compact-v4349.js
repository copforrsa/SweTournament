(()=>{
'use strict';
if(window.__SWE_TOURNAMENT_COMPACT_4349)return;window.__SWE_TOURNAMENT_COMPACT_4349=true;
...
async function generateFromTournament(button){
 ...
 if(!s||!id||!(typeof hasAdminOps==='function'&&(hasAdminOps()||(typeof isCoorg==='function'&&isCoorg()&&s.myPermissions?.can_generate_teams))))return;
 generating=true;button.disabled=true;
 try{s.activeTour=id;await loadTournament();if(String(s.activeTour)!==String(id))throw new Error('Le tournoi sélectionné a changé. Relance la génération.');await runSmartTeamGeneration();if(typeof renderAll==='function')renderAll()}
 catch(e){if(typeof toast==='function')toast(e.message||'Génération impossible')}
