...
async function load(){try{await resolveShortLink();if(!token)throw new Error('Paramètres du lien de saison incomplets.');const r=await sb.rpc('get_public_workspace_snapshot_v2',{p_token:token});if(r.error)throw r.error;const viewedSeasonId=render(r.data||{});const [rankings,view]=await Promise.all([sb.rpc('get_public_season_rankings',{p_public_token:token,p_season_id:viewedSeasonId}),sb.rpc('record_season_page_view',{p_public_token:token,p_season_id:viewedSeasonId})]);if(!rankings.error)renderOfficialRankings(rankings.data);if(!view.error){const count=Number(view.data||0);$('#seasonViews').textContent='👁️ '+count+' consultation'+(count>1?'s':'')+' de cette page'}else $('#seasonViews').textContent=''}catch(e){$('#seasonError').textContent=e.message||'Impossible de charger les classements.';$('#seasonError').classList.remove('hidden');$('#seasonStatus').textContent='Lien indisponible';$('#seasonViews').textContent=''}}
