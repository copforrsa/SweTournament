...
function statRows(rows,key,label,plus=false){return rows.length?rows.map(x=>'<div class="rank '+(x.guest?'guest-row':'')+'"><b>'+(x.rank===1?'👑':x.rank)+'</b><span><b>'+esc(x.name)+'</b>'+(x.guest?' <span class="guest-badge">Nouveau</span>':'')+(x.matches!=null?'<div class="muted">'+plural(x.matches,'match','matchs')+(x.g||x.a?' • ⚽ +'+x.g+' • 🎯 +'+x.a:'')+'</div>':'')+'</span><b class="right">'+(plus?'+':'')+x[key]+' '+label+(Number(x[key])>1?'s':'')+'</b></div>').join(''):'<p class="muted">Aucune donnée disponible pour cette saison.</p>'}
function expandable(boxId,buttonId,rows,key,label,plus=false){let expanded=false;const box=$(boxId),button=$(buttonId);const paint=()=>{const visible=expanded?rows:rows.slice(0,10);box.innerHTML=statRows(visible,key,label,plus);button.classList.toggle('hidden',rows.length<=10);button.textContent=expanded?'Réduire au Top 10':'Voir les autres ('+(rows.length-10)+')'};button.onclick=()=>{expanded=!expanded;paint()};paint()}
...
function render(data){
  ...
  const stats=new Map(players.map(p=>[String(p.id),{id:String(p.id),name:p.name,guest:p.is_group_member===false,g:0,a:0,total:0,matches:0,avg:0}]));
  goals.forEach(g=>{const scorer=stats.get(String(g.scorer_player_id));if(scorer)scorer.g++;const assister=stats.get(String(g.assister_player_id||''));if(assister)assister.a++});
  ...
  $('#seasonPodium').innerHTML=podiumOrder.length?podiumOrder.map(({player:x,className,medal})=>'<article class="podium-player '+className+'"><div class="podium-medal">'+medal+'</div><div class="podium-name">'+esc(x.name)+'</div><div class="podium-score">⭐ '+x.avg.toFixed(1)+'/10</div><div class="muted">'+plural(x.matches,'match','matchs')+' • ⚽ +'+x.g+' • 🎯 +'+x.a+'</div></article>').join(''):'<p class="muted">Le podium apparaîtra dès que les participations aux matchs seront enregistrées.</p>';
  expandable('#seasonScorers','#toggleSeasonScorers',scorers,'g','but',true);expandable('#seasonAssists','#toggleSeasonAssists',assists,'a','passe',true);
