...
function statRows(rows,key,label){return rows.length?rows.map(x=>'<div class="rank '+(x.guest?'guest-row':'')+'"><b>'+(x.rank===1?'👑':x.rank)+'</b><span><b>'+esc(x.name)+'</b>'+(x.guest?' <span class="guest-badge">Nouveau</span>':'')+(x.matches!=null?'<div class="muted">'+plural(x.matches,'match','matchs')+(x.g||x.a?' • ⚽ '+x.g+' • 🎯 '+x.a:'')+'</div>':'')+'</span><b class="right">'+x[key]+' '+label+(Number(x[key])>1?'s':'')+'</b></div>').join(''):'<p class="muted">Aucune donnée disponible pour cette saison.</p>'}
...
function render(data){
  ...
  $('#seasonScorers').innerHTML=statRows(scorers,'g','but');$('#seasonAssists').innerHTML=statRows(assists,'a','passe');
  $('#seasonTopPlayers').innerHTML=statRows(averages.map(x=>({...x,score:x.avg.toFixed(1)})),'score','point');
  $('#seasonNewPlayers').innerHTML=statRows(newcomers.map(x=>({...x,score:x.avg.toFixed(1)})),'score','point');
