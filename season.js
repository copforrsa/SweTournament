...
function statRows(rows,key,label){const deltaKey=key==='g'?'recentG':key==='a'?'recentA':null;return rows.length?rows.map(x=>'<div class="rank '+(x.guest?'guest-row':'')+'"><span class="rank-position">'+(x.rank===1?'♛':x.rank)+'</span><span class="rank-copy"><span class="rank-name">'+esc(x.name)+'</span>'+(x.guest?' <span class="guest-badge">Nouveau</span>':'')+(x.matches!=null?'<span class="rank-detail">'+plural(x.matches,'match','matchs')+(key==='score'&&(x.g||x.a)?' • ⚽ '+x.g+delta(x.recentG)+' • 🎯 '+x.a+delta(x.recentA):'')+'</span>':'')+'</span><span class="rank-value">'+x[key]+' '+label+(Number(x[key])>1?'s':'')+(deltaKey?delta(x[deltaKey]):'')+'<small>'+(key==='score'?'moyenne / 10':'sur la saison')+'</small></span></div>').join(''):'<p class="muted">Aucune donnée disponible pour cette saison.</p>'}
function expandable(boxId,buttonId,rows,key,label,groupLabel='joueurs'){let expanded=false;const box=$(boxId),button=$(buttonId);const paint=()=>{const visible=expanded?rows:rows.slice(0,10);box.innerHTML=statRows(visible,key,label);button.classList.toggle('hidden',rows.length<=10);button.textContent=expanded?'Réduire au Top 10':'Voir tous les '+groupLabel+' ('+(rows.length-10)+' autres)'};button.onclick=()=>{expanded=!expanded;paint()};paint()}
...
function render(data){
  ...
  $('#seasonPodium').innerHTML=podiumOrder.length?podiumOrder.map(({player:x,className,medal})=>'<article class="podium-player '+className+'"><div class="podium-place">'+(className==='first'?'Champion de la saison':className==='second'?'2e place':'3e place')+'</div><div class="podium-medal">'+medal+'</div><div class="podium-name">'+esc(x.name)+'</div><div class="podium-score"><strong>'+x.avg.toFixed(1)+'</strong><small>/ 10</small></div><div class="podium-stats"><span><b>'+x.matches+'</b> match'+(x.matches>1?'s':'')+'</span><span><b>⚽ '+x.g+delta(x.recentG)+'</b> buts</span><span><b>🎯 '+x.a+delta(x.recentA)+'</b> passes</span></div></article>').join(''):'<p class="muted">Le podium apparaîtra dès que les participations aux matchs seront enregistrées.</p>';
  expandable('#seasonScorers','#toggleSeasonScorers',scorers,'g','but','buteurs');expandable('#seasonAssists','#toggleSeasonAssists',assists,'a','passe','passeurs');
...
function renderOfficialRankings(data){
  const rows=(data?.players||[]).map(x=>({...x,recentG:Number(x.recent_g||0),recentA:Number(x.recent_a||0)}));
  const scorers=competitionRanks(rows.filter(x=>Number(x.g)>0).sort((a,b)=>b.g-a.g||b.a-a.a||a.name.localeCompare(b.name,'fr')),x=>Number(x.g));
  const assists=competitionRanks(rows.filter(x=>Number(x.a)>0).sort((a,b)=>b.a-a.a||b.g-a.g||a.name.localeCompare(b.name,'fr')),x=>Number(x.a));
  expandable('#seasonScorers','#toggleSeasonScorers',scorers,'g','but','buteurs');
  expandable('#seasonAssists','#toggleSeasonAssists',assists,'a','passe','passeurs');
}
async function load(){try{await resolveShortLink();if(!token)throw new Error('Paramètres du lien de saison incomplets.');const r=await sb.rpc('get_public_workspace_snapshot_v2',{p_token:token});if(r.error)throw r.error;const viewedSeasonId=render(r.data||{});window.swePageViewContext={page:'season',token,id:viewedSeasonId};document.dispatchEvent(new Event('swe:page-view'));const rankings=await sb.rpc('get_public_season_rankings',{p_public_token:token,p_season_id:viewedSeasonId});if(!rankings.error)renderOfficialRankings(rankings.data);$('#seasonViews').textContent=''}catch(e){$('#seasonError').textContent=e.message||'Impossible de charger les classements.';$('#seasonError').classList.remove('hidden');$('#seasonStatus').textContent='Lien indisponible';$('#seasonViews').textContent=''}}
