...
function roleForMatch(t,m){
  const saved=String(m?.rotation_role||'');
...
R.launch=async function(t){if(!t||t.rotation_mode!=='king_of_pitch')throw new Error('Ce tournoi n’est pas en format Roi du terrain.');const roles=rolesFor(t);const teams=[...(S.teams||[])].filter(x=>String(x.tournament_id)===String(t.id)||!x.tournament_id);if(roles.length<1)throw new Error('Configure au moins le terrain du Roi.');if(teams.length>7)throw new Error('Le moteur Roi du terrain gère actuellement jusqu’à 7 équipes.');if(teams.length>=6&&roles.length<3)throw new Error('Avec 6 ou 7 équipes, configure 3 terrains.');if(teams.length>=4&&teams.length<=5&&roles.length<2)throw new Error('Avec 4 ou 5 équipes, configure Roi + Ruisseau.');if(teams.length<2)throw new Error('Il faut au moins 2 équipes.');let s=stateOf(t);if(s.initialized)throw new Error('Le circuit est déjà lancé.');const first=[...(S.matches||[])].filter(m=>String(m.tournament_id)===String(t.id)&&String(m.status||'scheduled')!=='finished'&&!m.rotation_generated).sort((a,b)=>Number(a.match_order||0)-Number(b.match_order||0));if(first.length!==roles.length)throw new Error('Crée exactement '+roles.length+' premier'+(roles.length>1?'s':'')+' match'+(roles.length>1?'s':'')+' manuellement, un par terrain, avant de lancer le circuit.');const byRole=new Map();for(const m of first){const role=roles.find(r=>String(pitchName(t,r)).trim().toLowerCase()===String(m.pitch||'').trim().toLowerCase());if(!role||byRole.has(role))throw new Error('Chaque match de départ doit être créé sur un terrain différent avec un rôle configuré.');byRole.set(role,m)}if(byRole.size!==roles.length)throw new Error('Il manque un match de départ sur l’un des terrains configurés.');const used=new Set(first.flatMap(m=>[String(m.home_team_id),String(m.away_team_id)]));if(used.size!==first.length*2)throw new Error('Une équipe ne peut jouer qu’un seul premier match. Corrige les matchs de départ.');s={version:1,initialized:true,team_count:teams.length,queue:teams.filter(x=>!used.has(String(x.id))).map(x=>x.id),active:{},king_holder_team_id:byRole.get('king').home_team_id,waiting:{king_winners:[],king_losers:[],middle_winners:[],middle_losers:[],stream_winners:[],stream_losers:[],bottom_entries:[]}};for(const role of roles){const m=byRole.get(role),row={status:'in_progress',started_at:new Date().toISOString(),rotation_role:role,rotation_generated:true,pitch:pitchName(t,role),round_label:role==='king'?'👑 Roi':role==='middle'?'↕️ Intermédiaire':'🌊 Ruisseau'};const r=await sb.from('matches').update(row).eq('id',m.id).select('*').single();if(r.error)throw r.error;Object.assign(m,r.data);s.active[role]=m.id;if(role==='king')s.king_holder_team_id=m.home_team_id;document.dispatchEvent(new CustomEvent('swe:rotation-match-created',{detail:{match:m,role}}))}await saveState(t,s);if(typeof renderMatches==='function')renderMatches();document.dispatchEvent(new Event('swe:rendered'));return s}
R.finish=async function(m,shifumiWinnerId=null){const t=(S.tournaments||[]).find(x=>String(x.id)===String(m.tournament_id))||(typeof currentTour==='function'?currentTour():null);if(!t||t.rotation_mode!=='king_of_pitch')throw new Error('Rotation Roi du terrain inactive.');const role=roleForMatch(t,m);if(!role)throw new Error('Rôle du terrain introuvable.');const s=stateOf(t);if(!s.initialized)throw new Error('Lance le circuit Roi du terrain avant de terminer ses matchs.');const hs=Number(m.home_score||0),as=Number(m.away_score||0);let winner=null;if(hs>as)winner=m.home_team_id;else if(as>hs)winner=m.away_team_id;else if(role==='king')winner=s.king_holder_team_id||m.home_team_id;else winner=shifumiWinnerId;if(!winner)throw new Error('SHIFUMI_REQUIRED');const loser=String(winner)===String(m.home_team_id)?m.away_team_id:m.home_team_id;const up=await sb.from('matches').update({status:'finished',finished_at:new Date().toISOString(),rotation_role:role}).eq('id',m.id);if(up.error)throw up.error;m.status='finished';m.finished_at=new Date().toISOString();s.active[role]=null;q(s,role+'_winners').push(winner);q(s,role+'_losers').push(loser);
