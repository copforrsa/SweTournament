...
async function schedule2(t,s){const odd=s.team_count%2===1;
 if(odd){
   if(!s.active.king&&q(s,'king_winners').length){
     if(q(s,'middle_winners').length){const kw=q(s,'king_winners').shift(),mw=q(s,'middle_winners').shift(),kl=q(s,'king_losers').shift()||null;await createMatch(t,s,'king',kw,mw);if(kl)s.queue.push(kl)}
     else if(s.active.middle&&s.queue.length){const kw=q(s,'king_winners').shift(),outside=s.queue.shift(),kl=q(s,'king_losers').shift()||null;await createMatch(t,s,'king',kw,outside);if(kl)s.queue.push(kl)}
   }
   if(!s.active.middle&&q(s,'middle_losers').length&&s.queue.length){const ml=q(s,'middle_losers').shift(),outside=s.queue.shift();await createMatch(t,s,'middle',ml,outside)}
 }else{
   ...
   if(!s.active.middle&&q(s,'king_losers').length&&q(s,'middle_losers').length)await createMatch(t,s,'middle',q(s,'king_losers').shift(),q(s,'middle_losers').shift());
 }
}
