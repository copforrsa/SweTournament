...
R.launch=async function(t){if(!t||t.rotation_mode!=='king_of_pitch')throw new Error('Ce tournoi n’est pas en format Roi du terrain.');const roles=rolesFor(t);const teams=[...(S.teams||[])].filter(x=>String(x.tournament_id)===String(t.id)||!x.tournament_id);if(roles.length<1)throw new Error('Configure au moins le terrain du Roi.');if(teams.length>7)throw new Error('Le moteur Roi du terrain gère actuellement jusqu’à 7 équipes.');if(teams.length>=6&&roles.length<3)throw new Error('Avec 6 ou 7 équipes, configure 3 terrains.');if(teams.length>=4&&teams.length<=5&&roles.length<2)throw new Error('Avec 4 ou 5 équipes, configure Roi + Ruisseau.');if(teams.length<2)throw new Error('Il faut au moins 2 équipes.');let s=stateOf(t);if(s.initialized)throw new Error('Le circuit est déjà lancé.');const first=[...(S.matches||[])].filter(m=>String(m.tournament_id)===String(t.id)&&String(m.status||'scheduled')!=='finished'&&!m.rotation_generated).sort((a,b)=>Number(a.match_order||0)-Number(b.match_order||0));if(first.length!==roles.length)throw new Error('Crée exactement '+roles.length+' premier'+(roles.length>1?'s':'')+' match'+(roles.length>1?'s':'')+' manuellement, un par terrain, avant de lancer le circuit.');const used=new Set(first.flatMap(m=>[String(m.home_team_id),String(m.away_team_id)]));if(used.size!==first.length*2)throw new Error('Une équipe ne peut jouer qu’un seul premier match. Corrige les matchs de départ.');s={version:1,initialized:true,team_count:teams.length,queue:teams.filter(x=>!used.has(String(x.id))).map(x=>x.id),active:{},king_holder_team_id:first[0].home_team_id,waiting:{king_winners:[],king_losers:[],middle_winners:[],middle_losers:[],stream_winners:[],stream_losers:[],bottom_entries:[]}};for(let i=0;i<roles.length;i++){const role=roles[i],m=first[i],row={status:'in_progress',started_at:new Date().toISOString(),rotation_role:role,rotation_generated:true,pitch:pitchName(t,role),round_label:role==='king'?'👑 Roi':role==='middle'?'↕️ Intermédiaire':'🌊 Ruisseau'};const r=await sb.from('matches').update(row).eq('id',m.id).select('*').single();if(r.error)throw r.error;Object.assign(m,r.data);s.active[role]=m.id;if(role==='king')s.king_holder_team_id=m.home_team_id;document.dispatchEvent(new CustomEvent('swe:rotation-match-created',{detail:{match:m,role}}))}await saveState(t,s);if(typeof renderMatches==='function')renderMatches();document.dispatchEvent(new Event('swe:rendered'));return s}
