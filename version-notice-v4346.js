(()=>{
'use strict';
if(window.__SWE_VERSION_NOTICE_4375)return;window.__SWE_VERSION_NOTICE_4375=true;
...
async function checkLatest(){
 ...
 try{
  ...
  const button=document.createElement('button');button.type='button';button.textContent='Actualiser';button.onclick=()=>location.reload();panel.append(label,button);document.body.append(panel);
 }catch(_){}finally{checking=false;}
}
window.addEventListener('pageshow',checkLatest);document.addEventListener('visibilitychange',checkLatest);setInterval(checkLatest,60000);
